...
(function(){
  ...
  function projectStats(q,st){var rows=st.filter(function(s){return s.qualification_id===q.id}),total=rows.reduce(function(a,s){return a+Number(s.minutes||0)},0),dates=rows.map(function(s){return s.study_date}).sort(),start=dates[0]||(q.created_at&&q.created_at.slice(0,10));return {total:total,start:start,days:[].filter.call(dates,function(v,i,a){return a.indexOf(v)===i}).length}}
  ...
  function renderWeekly(st,q){
    ...
    for(var bi=0;bi<7;bi++){var day=new Date(d.ws.getFullYear(),d.ws.getMonth(),d.ws.getDate()+bi),key=iso(day),cur=d.map[key]||0,prev=d.map[iso(new Date(day.getFullYear(),day.getMonth(),day.getDate()-7))]||0;bars+='<div class="sd-day"><div class="sd-bars"><div class="sd-prev" style="height:'+Math.min(100,Math.round(prev/180*100))+'%"></div><div class="sd-current" style="height:'+Math.min(100,Math.round(cur/180*100))+'%"></div></div><div class="sd-min">'+cur+'分</div><div class="sd-label">'+['月','火','水','木','金','土','日'][bi]+'</div></div>'}
    var projects=q.map(function(qq){var ps=projectStats(qq,st),exam=qq.exam_date?Math.ceil((parseDate(qq.exam_date)-d.baseDate)/86400000):null;return {q:qq,p:ps,exam:exam}}).filter(function(x){return x.p.total>0||x.q.exam_date}).sort(function(a,b){return b.p.total-a.p.total});
    var projectHtml=projects.length?projects.map(function(x){return '<div class="sd-project"><div><div class="sd-project-name">'+escapeHtml(x.q.name)+'</div><div class="muted small">'+(x.p.start?'開始 '+fmtDate(x.p.start):'学習記録なし')+(x.exam!=null?' ・ '+(x.exam>=0?'試験まで '+x.exam+'日':'試験済み'):'')+'</div></div><div class="sd-project-time">'+minutesText(x.p.total)+'</div><div class="sd-project-sub">学習日 '+x.p.days+'日</div></div>'}).join(''):'<div class="item">まだ学習プロジェクトがありません。</div>';
    return '<div class="sd-wrap"><div class="sd-section-head"><div><div class="kicker">WEEKLY STUDY</div><h3>今週の学習時間</h3><div class="muted small">濃い棒が今週、薄い棒が先週。</div></div><span class="badge">'+d.start.replace(/-/g,'.')+' 〜 '+d.end.slice(5).replace('-','.')+'</span></div><div class="sd-chart">'+bars+'</div><div class="sd-summary"><div><span>今週</span><strong>'+minutesText(d.weekTotal)+'</strong></div><div><span>1日平均</span><strong>'+Math.round(d.weekTotal/7)+'分</strong></div><div><span>先週比</span><strong>'+trend+'</strong></div></div><div class="sd-compare"><div class="sd-compare-card"><span>先週の総合計</span><b>'+minutesText(d.prevTotal)+'</b></div><div class="sd-compare-card"><span>今週の学習日数</span><b>'+d.activeThis+'日</b><small>7日中</small></div></div><div class="sd-project-head"><div><div class="kicker">PROJECTS</div><h3>学習プロジェクト</h3></div></div>'+projectHtml+'</div>';
  }
  ...
  function renderMonthly(st){
    var base=parseDate(studyDay()),end=monthStart(base),months=[],map={};
    st.forEach(function(s){if(s.study_date){var d=parseDate(s.study_date),k=monthKey(d);map[k]=(map[k]||0)+Number(s.minutes||0)}});
    for(var i=11;i>=0;i--){var d=addMonths(end,-i);months.push({key:monthKey(d),label:monthLabel(d),minutes:map[monthKey(d)]||0})}
    var current=months[11].minutes,previous=months[10].minutes,change=pctChange(current,previous),trend=change==null?'—':(change>0?'＋':'')+change+'%',total=months.reduce(function(a,m){return a+m.minutes},0),active=months.filter(function(m){return m.minutes>0}).length;
    var max=months.reduce(function(a,m){return Math.max(a,m.minutes)},0)||1,bars=months.map(function(m){return '<div class="sd-day"><div class="sd-bars"><div class="sd-current" style="height:'+Math.max(m.minutes?2:0,Math.min(100,Math.round(m.minutes/max*100)))+'%"></div></div><div class="sd-min">'+m.minutes+'分</div><div class="sd-label">'+m.label+'</div></div>'}).join('');
    return '<div class="sd-wrap"><div class="sd-section-head"><div><div class="kicker">MONTHLY STUDY</div><h3>月別の学習時間</h3><div class="muted small">直近12か月の学習時間。</div></div><span class="badge">'+months[0].label+' 〜 '+months[11].label+'</span></div><div class="sd-chart sd-month-chart">'+bars+'</div><div class="sd-summary"><div><span>今月</span><strong>'+minutesText(current)+'</strong></div><div><span>月平均</span><strong>'+minutesText(Math.round(total/12))+'</strong></div><div><span>先月比</span><strong>'+trend+'</strong></div></div><div class="sd-compare"><div class="sd-compare-card"><span>直近12か月の総合計</span><b>'+minutesText(total)+'</b></div><div class="sd-compare-card"><span>学習した月</span><b>'+active+'か月</b><small>12か月中</small></div></div></div>';
  }
  ...
  function setViewButtons(mode){
    ...
    row.innerHTML='<button type="button" class="'+(mode==='qualification'?'primary':'light')+'" data-dashboard-mode="qualification">資格一覧</button><button type="button" class="'+(mode==='weekly'?'primary':'light')+'" data-dashboard-mode="weekly">週・月</button><button type="button" class="'+(mode==='qualification-chart'?'primary':'light')+'" data-dashboard-mode="qualification-chart">資格別</button>';
    var active=document.querySelector('#dashboardPeriodToggle');
  ...
  function renderDashboardMode(mode){
    ...
    if(mode==='monthly')box.innerHTML=renderMonthly(st);else if(mode==='qualification-chart')box.innerHTML=renderQualification(st,q);else if(mode==='weekly')box.innerHTML=renderWeekly(st,q);else box.innerHTML=renderWeekly(st,q);
    setViewButtons(mode==='monthly'?'monthly':mode==='qualification-chart'?'qualification-chart':mode==='weekly'?'weekly':'qualification');
  }
  function renderStudyDashboard(){renderDashboardMode('weekly')}
  ...
  function patch(){
    ...
    try{window.showDashboard=function(){renderDashboardMode('qualification')}}catch(_){}
