(function(){
  ...
  // 「今日」の境界を午前5時に統一。復習・連続学習・試験カウントダウン等が同じ日付基準になる。
  ...
  function projectStats(q,st){var rows=st.filter(function(s){return s.qualification_id===q.id}),total=rows.reduce(function(a,s){return a+Number(s.minutes||0)},0),dates=rows.map(function(s){return s.study_date}).sort(),start=dates[0]||(q.created_at&&q.created_at.slice(0,10)),days=0;if(start)days=Math.max(1,Math.floor((parseDate(studyDay())-parseDate(start))/86400000)+1);return {total:total,start:start,avg:days?Math.round(total/days):0,days:dates.length}}
  function renderStudyDashboard(){
    var box=document.getElementById('dashboard');if(!box)return;
    var st=(typeof sessions!=='undefined'&&Array.isArray(sessions))?sessions:[],q=(typeof qualifications!=='undefined'&&Array.isArray(qualifications))?qualifications:[],now=new Date(),baseDay=studyDay(now),baseDate=parseDate(baseDay),ws=weekStart(baseDate),start=iso(ws),end=iso(new Date(ws.getFullYear(),ws.getMonth(),ws.getDate()+6)),prevStart=iso(new Date(ws.getFullYear(),ws.getMonth(),ws.getDate()-7)),prevEnd=iso(new Date(ws.getFullYear(),ws.getMonth(),ws.getDate()-1));
    ...
    var lastMonth=new Date(baseDate.getFullYear(),baseDate.getMonth()-1,1),lmStart=iso(lastMonth),lmEnd=iso(new Date(baseDate.getFullYear(),baseDate.getMonth(),0)),lastMonthTotal=sumRange(st,lmStart,lmEnd),lmDays=0;
    for(var ld=1;ld<=new Date(baseDate.getFullYear(),baseDate.getMonth(),0).getDate();ld++)if((map[iso(new Date(lastMonth.getFullYear(),lastMonth.getMonth(),ld))]||0)>0)lmDays++;
    var thisAvg=Math.round(weekTotal/7),prevAvg=Math.round(prevTotal/7),lastMonthAvg=lmDays?Math.round(lastMonthTotal/lmDays):0,change=pctChange(weekTotal,prevTotal),trend=change==null?'—':(change>0?'＋':'')+change+'%',bars='';
    for(var bi=0;bi<7;bi++){var d=new Date(ws.getFullYear(),ws.getMonth(),ws.getDate()+bi),key=iso(d),cur=map[key]||0,prevKey=iso(new Date(d.getFullYear(),d.getMonth(),d.getDate()-7)),prev=map[prevKey]||0;bars+='<div class="sd-day"><div class="sd-bars"><div class="sd-prev" style="height:'+Math.min(100,Math.round(prev/180*100))+'%"></div><div class="sd-current" style="height:'+Math.min(100,Math.round(cur/180*100))+'%"></div></div><div class="sd-min">'+(cur||0)+'分</div><div class="sd-label">'+['月','火','水','木','金','土','日'][bi]+'</div></div>'}
    var projects=q.map(function(qq){var ps=projectStats(qq,st),exam=qq.exam_date?Math.ceil((parseDate(qq.exam_date)-parseDate(baseDay))/86400000):null;return {q:qq,p:ps,exam:exam}}).filter(function(x){return x.p.total>0||x.q.exam_date}).sort(function(a,b){return b.p.total-a.p.total});
  ...
  function hook(){
    ...
    setTimeout(function(){
      window.today=studyDay;
      var el=document.getElementById('recordDate');if(el)el.value=studyDay();
      if(document.getElementById('tab-status')?.style.display!=='none')renderStudyDashboard();
      hook();
