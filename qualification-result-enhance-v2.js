(function(){
  ...
  window.updateQualificationResultAware=async function(id){
    ...
    var remove=old.filter(function(s){return !names.includes(s.name)});if(remove.length){var rr=await sb.from('subjects').update({archived:true}).in('id',remove.map(function(s){return s.id})).eq('id',remove.map(function(s){return s.id}));if(rr.error){alert('科目の更新に失敗しました: '+rr.error.message);return}}
