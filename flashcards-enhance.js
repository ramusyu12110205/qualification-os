...
(function(){
  ...
  window.loadMasters=async function(){
    const q=await sb.from('qualifications').select('id,name').order('name');
    if(q.error)return toast(q.error.message);
    window.qualifications=q.data||[];
    const s=await sb.from('subjects').select('id,name,qualification_id').order('name');
    if(s.error)return toast(s.error.message);
    window.subjects=s.data||[];
  };

  window.renderSubjectOptions=function(selected){
    const q=document.getElementById('deckQualification');
    const s=document.getElementById('deckSubject');
    if(!q||!s)return;
    const qid=q.value||'';
    s.innerHTML='<option value="">'+(qid?'科目を選択':'資格を先に選択')+'</option>';
    (window.subjects||[]).filter(x=>x.qualification_id===qid).forEach(x=>{
      const o=document.createElement('option');
      o.value=x.id;o.textContent=x.name;o.selected=x.id===selected;s.appendChild(o);
    });
  };

  window.showDeckForm=function(d=null){
    hideAll();
    const e=document.getElementById('deckForm');
    e.classList.remove('hidden');e.dataset.id=d?.id||'';
    e.innerHTML='<h2>'+(d?'ファイル編集':'ファイル作成')+'</h2>'+
      ...
      '<label>連携する資格</label><select id="deckQualification" onchange="renderSubjectOptions()"><option value="">連携しない</option>'+((window.qualifications||[]).map(q=>'<option value="'+q.id+'">'+esc(q.name)+'</option>').join(''))+'</select>'+
      '<label>科目</label><select id="deckSubject"></select>'+ 
      '<label>4択の判定方式</label><div class="row">'+
      '<label style="font-weight:500"><input type="radio" name="questionMode" value="one_to_one" checked> 1対1対応</label>'+ 
      '<label style="font-weight:500"><input type="radio" name="questionMode" value="shared_choices"> 同じ答えを共有</label></div>'+ 
      '<p class="muted small">同じ答えを共有：答えが2～3種類でも、その種類数に合わせて2～3択で出題します。</p>'+ 
    ...
    const q=document.getElementById('deckQualification');
    q.value=d?.qualification_id||'';
    renderSubjectOptions(d?.subject_id||'');
    const r=document.querySelector('input[name="questionMode"][value="'+(d?.question_mode||'one_to_one')+'"]');
    if(r)r.checked=true;
  };

  window.saveDeck=async function(){
    const name=document.getElementById('deckName').value.trim();
    if(!name)return toast('ファイル名を入力してください');
    const id=document.getElementById('deckForm').dataset.id;
    const payload={
      name,
      description:document.getElementById('deckDescription').value.trim(),
      qualification_id:document.getElementById('deckQualification').value||null,
      subject_id:document.getElementById('deckSubject').value||null,
      question_mode:document.querySelector('input[name="questionMode"]:checked')?.value||'one_to_one'
    };
    const query=id?sb.from('flashcard_decks').update(payload).eq('id',id).eq('user_id',user.id):sb.from('flashcard_decks').insert({...payload,user_id:user.id});
  ...
  window.startSession=async function(mode){
    if(mode==='choice'&&currentDeck?.question_mode==='shared_choices'){
      if(currentCards.length<2)return toast('グループ化の4択には2枚以上のカードが必要です');
      const unique=[...new Set(currentCards.map(c=>c.answer))];
      if(unique.length<2)return toast('異なる答えが2種類以上必要です');
    ...
    const{data,error}=await sb.from('flashcard_sessions').insert({user_id:user.id,deck_id:currentDeck.id,mode}).select().single();
    if(error)return toast(error.message);
    session=data;sessionCards=[...currentCards].sort(()=>Math.random()-.5);idx=0;answerShown=false;paused=false;elapsed=0;sessionCorrect=0;sessionDoneCount=0;startedAt=Date.now();
    hideAll();$('session').classList.remove('hidden');renderSession();
    timerHandle=setInterval(()=>{if(!paused){elapsed=Math.floor((Date.now()-startedAt)/1000);if($('timer'))$('timer').textContent=fmt(elapsed)}},500);
  };
  ...
  window.renderChoice=function(){
    ...
    if(currentDeck.question_mode==='shared_choices'){
      ...
      opts=answers.map((answer,i)=>({id:'answer-'+i,answer}));
    }else{
      opts=[c,...currentCards.filter(x=>x.id!==c.id).sort(()=>Math.random()-.5).slice(0,3)].sort(()=>Math.random()-.5).map(x=>({id:x.id,answer:x.answer}));
    }
  ...
  window.choose=async function(id){
    ...
    let ok=false;
    if(currentDeck.question_mode==='shared_choices'){
      const b=[...document.querySelectorAll('#body button')].find(x=>x.getAttribute('onclick')==="choose('"+id+"')");
      ok=!!b&&b.textContent.trim()===c.answer;
    }else ok=id===c.id;
    sessionCorrect+=ok?1:0;
    if(!await recordCard(ok?'correct':'incorrect'))return;
    sessionDoneCount++;
  ...
  window.loadHistory=async function(){
    const{data,error}=await sb.from('flashcard_sessions').select('*,flashcard_decks(name)').order('created_at',{ascending:false}).limit(10);
    if(error)return toast(error.message);
  ...
  window.endSession=async function(){
    ...
    if(!paused)elapsed=Math.floor((Date.now()-startedAt)/1000);
    ...
    if(currentDeck.qualification_id&&currentDeck.subject_id){
      ...
      const memo='暗記カード：'+currentDeck.name+'（'+(session.mode==='choice'?'4択':'自由回答')+'）';
      ...
      if(error)return toast('資格勉強OSへの同期に失敗しました：'+error.message);
      studyRecordId=data.id;
    }
    ...
    if(error)return toast(error.message);
    session=null;
    toast(studyRecordId?'暗記セッションを記録・資格勉強OSへ同期しました':'暗記セッションを記録しました');
    await loadHistory();
